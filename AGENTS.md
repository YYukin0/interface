...
| D4 | **The Anthropic Messages protocol with strict tool-calling**, in discovery only | Structured action output, never free text. The client is `@anthropic-ai/sdk`, but the model is configuration (`CUA_LLM_MODEL`/`CUA_LLM_BASE_URL`): the two committed runs used `k3` and `kimi-k2.7-code-highspeed` against Moonshot's Anthropic-compatible endpoints. Replay must never import an LLM client (see I1). |
...
| D11 | **Cross-tenant reuse via sparse overrides, plus the one counter that serves both remaining stretch goals** | Cross-tenant reuse satisfies a core evaluation criterion (§3.7 generalization) and a stretch goal at once. `.stability.json` then covers multi-run stability and confidence/approval with a single mechanism — see `REPORT.md` §7 for why those two are not breadth-chasing. |
