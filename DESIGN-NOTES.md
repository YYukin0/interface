...
The long form. [REPORT.md](REPORT.md) is the brief's write-up and answers the same
seven questions in the same order; this is what each of those answers is a summary
of — the arguments in full, the measurements behind the numbers, and the bugs that
produced the design. Read REPORT.md first; come here for a section you want to
argue with.
...
runs **282 tests across ten workspaces**; the numbers quoted in each section are
...
| `discovery` | the agent loop; the **only** package permitted a model SDK | 56 |
...
| `evidence` | one directory per run, redacted on the way to disk | 16 |
| `handoff` | control lease, console server, audit record | 32 |
| `apps/legacy-app` | the hostile target: framesets, table layout, no test ids, six injectable faults | 15 |
